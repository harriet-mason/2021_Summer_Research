...
```{r}
bignumbats_scags_wide <- readRDS("C:/Users/harri/OneDrive/2021_Summer_Research/wholesetscags.rds")
...
```
## Absolute dist
- Aren'y really any scagnostics where it sits away from the general "trend" of the data. Mostly would need to use individual or combinations of scagnsotics.
# Scaled Dist
...
#make wide data
bignumbats_scags_wide_out <- bignumbats_scags_long %>%
  select(c(Var1, Var2, scag, group_diff)) %>%
  pivot_wider(id_cols=c(Var1, Var2), names_from=scag, values_from = group_diff)

#save data
saveRDS(bignumbats_scags_wide_out, "wholesetscags_outlier.rds")

```

### Redo SPLOM and top-scags checks
```{r}
bignumbats_scags_wide_out <- readRDS("C:/Users/harri/OneDrive/2021_Summer_Research/wholesetscags_outliers.rds")
...
animate_xy(numbat_subset2[, 1:10], guided_tour(tourr_splines), sphere = TRUE) #sometimes?
...
animate_xy(numbat_subset2[, 1:10], guided_tour(tourr_sparse), sphere = TRUE)
animate_xy(numbat_subset2[, 1:10], guided_tour(tourr_monotonic), sphere = TRUE)
animate_xy(numbat_subset2[, 1:10], guided_tour(tourr_outlying), sphere = TRUE)
```
...
#try tourr with this
animate_xy(numbat_subset[, 1:10], guided_tour(tourr_dcor(cl=numbat_subset$group)), 
           sphere = TRUE, , col = numbat_subset$group)
