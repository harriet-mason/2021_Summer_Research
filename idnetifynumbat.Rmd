...
# Threshold function
scag_threshold <- function(scagnostic, value){
  #set thresholds
  thresholds <- list(
    "clumpy2" = 0.5,
  ...
  #return value
  if (scagnsotic=="convex") ifelse(A_value > thesholds[scagnostic], 0, A_value)
  else ifelse(A_value < thesholds[scagnostic], 0, A_value)
...
bignumbats_scags_long_dist <- bignumbatsA_scags_outrm_long %>%
  ...
  mutate(A_value = ifelse(A_value < 0.1, 0, A_value),
         B_value = ifelse(B_value < 0.1, 0, B_value),
         # calc group difference
         group_diff = (A_value - B_value)/(A_value + B_value))
