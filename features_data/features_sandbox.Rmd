...
```{r, Working Version From Playing Around}
...
#example
filename <- readRDS("features_data/tours/tour_convex_bet_0.5_1_.rds")
...
grid_tourr <- function(numeric_data, group_vector, scags, searches, seeds, alpha, ...){
  expand.grid(scags, searches, seeds, alpha, stringsAsFactors=FALSE) %>%
    mutate(filename = paste("features_data/tours2/tour", Var1, Var2, Var4, Var3, ".rds", sep="_")) %>%
    group_by(Var1, Var2, Var4, Var3) %>%
    summarise(save_tourr(numeric_data, group_vector, scag = Var1,  search = Var2, seed = Var3, 
                         alpha = Var4, filename = filename))
}
