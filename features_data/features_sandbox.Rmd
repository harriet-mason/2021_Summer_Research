...
# filename = paste("features_data/tours/tour_convex_geodesic_seed", i, ".rds", sep="")
save_tourr <- function(numeric_data, group_vector, scag, search){
  indx <- list(
  ...
  save_history(numeric_data,
               tour_path = guided_tour(unlist(unname(indx[scag])), d=2,
                                     search_f =  srch[search]))
}

save_tourr(feature_vs_noise[1:5], feature_vs_noise$group, "convex", "geo")

# test example
list_sum <- function(func, numbers){
  fncs <- list(
    "smm" = sum(numbers),
    "sqr" = numbers^2
  )
  unlist(unname(fncs[func]))
}

list_sum("smm", c(1,2,3,4,5))
list_sum("sqr", c(1,2,3,4,5))
...
m <- save_tourr(feature_vs_noise[1:5],
...
convex_better <- animate_xy(feature_vs_noise[1:5],
                            tour_path = planned_tour(m),
                            col = feature_vs_noise$group)

convex_better %>%
  explore_trace_interp() + 
  scale_color_continuous_botanical() + 
  ggtitle(paste("Feature vs Noise: Convex/Better/Seed_"))
```
