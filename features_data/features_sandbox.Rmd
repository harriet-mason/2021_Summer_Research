...
save_tourr <- function(numeric_data, group_vector, scag, search, seed, delta_alpha, filename, ...){
  ...
  #example
save_tourr(feature_vs_noise[1:5], feature_vs_noise$group, "skinny", "geo", "goodvibes.rds", delta_alpha=0.1)
...
grid_tourr(numeric_data = feature_vs_noise[1:5], 
           ...
           scags = c("convex"), #scags = c("convex", "skinny"), 
           searches = c("geo", "bet"), #searches = c("geo", "bet", "bet_rand"), 
           seeds = seq(1), #seeds = seq(5), 
           delta_alpha = c(0.1, 0.7)) #delta_alpha = c(0.05, 0.1, 0.5, 0.7, 1)) 
...
#example
filename <- readRDS("features_data/tours/tour_skinny_bet_2_.rds")
